...
public final class AgentResponseOverlay {
    ...
    public static void handleStreamEvent(String requestId, AgentStreamEventType type, String payload) {
        MineClawd.LOGGER.info("[StreamEvent] handleStreamEvent called: type={}, payload_len={}", type, payload != null ? payload.length() : 0);
        if (!isOverlayEnabled()) {
            MineClawd.LOGGER.info("[StreamEvent] Overlay not enabled, skipping further processing");
            return;
        }
    ...
    public static boolean charTyped(MinecraftClient client, char character, int keyCode) {
        ...
        // When input is not focused or input bar is not shown, let char events pass through
        // so the user can type in chat or other underlying screens
    ...
    public static boolean keyPressed(MinecraftClient client, int keyCode, int scanCode, int modifiers) {
        ...
        // When the input bar is not shown or not focused, let key events pass through
        // so the user can type in chat or interact with other underlying screens
    ...
    public static boolean capturesKeyboardInput(MinecraftClient client) {
        // Only capture keyboard input when an input field in the overlay is actively focused,
        // so the user can still type in chat or interact with underlying screens when not focused
        if (!isOverlayEnabled() || !isInteractive(client) || !visible || minimized) {
            return false;
        }
        return inputFocused || questionOtherFocused;
    }
