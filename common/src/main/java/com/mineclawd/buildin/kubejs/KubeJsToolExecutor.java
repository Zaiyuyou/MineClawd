...
public final class KubeJsToolExecutor {
    ...
    /**
     * 询问用户问题（异步工具）
     * 这是一个包装器方法，实际实现在 MineClawd.java 中
     */
    public static ToolExecutionResult askUserQuestion(ServerCommandSource source, String question, List<String> options) {
        // 由于 ask-user-question 是异步工具，实际的异步处理在 MineClawd.java 中
        // 这里返回一个占位结果，表示工具已调用
        return new ToolExecutionResult(true, "ask-user-question tool called. Async handling in MineClawd.java.");
    }
